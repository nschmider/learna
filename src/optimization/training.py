...
from src.learna.agent import (AgentConfig,
                              ppo_agent_kwargs,
                              get_agent,
                              NetworkConfig,
                              get_network)
...
def make_synthetic_data(amount, length):
    ...
    for _ in range(amount):
        ...
        for i in range(length):
            if dot_bracket[i] == ")":
                continue
            if not any([dot_bracket[j] == "." for j in range(i+1, length)]):
                # Checks if you can still make 2 brackets
                continue
            make_bracket = np.random.choice([True, False], p=[0.2, 0.8])
            if make_bracket:
                # Returns an index that is not assigned yet,
                # every index has uniform probabilities
                possible_indices = [j for j in range(i+1, length)
                                    if dot_bracket[j] == "."]
                second_bracket = np.random.choice(possible_indices)
                dot_bracket = dot_bracket[:i] + "(" + dot_bracket[i+1:]
                dot_bracket = (dot_bracket[:second_bracket] +
                               ")" + dot_bracket[second_bracket+1:])
...
def evaluate(env_config, agent, dot_brackets, tries):
    ...
    environment = RnaDesignEnvironment(dot_brackets=dot_brackets,
...
if __name__ == "__main__":
    ...
    env_config = RnaDesignEnvironmentConfig(reward_exponent=9.34,
                                            state_radius=32)
    ...
    num_episodes = 1000
    rewards = training(env_config,
                       agent_config,
                       network_config,
                       dot_brackets,
                       num_episodes)
    rewards = [np.mean(rewards[i:i+100])
               for i in range(0, len(rewards), num_episodes // 100)]
